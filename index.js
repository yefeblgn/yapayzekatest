...
function loadConfig() {
    ...
    DISCORD_TOK = process.env.token || DISCORD_TOK;
    WITAI_TOK = process.env.yapayzekaid || WITAI_TOK;
    SPEECH_METHOD = process.env.speech || SPEECH_METHOD;
    ...
    if (SPEECH_METHOD === 'google' && !fs.existsSync('./vcxbbhfg-ce84bcb0e6c8.json'))
    ...
    try {
        ...
        if (msg.content.trim().toLowerCase() == _CMD_JOIN) {
            if (!msg.member.voice.channelID) {
                msg.reply('Hata: lütfen önce bir ses kanalına katılın.')
            } else {
                if (!guildMap.has(mapKey))
                    await connect(msg, mapKey)
                else
                    msg.reply('Zaten bağlandım.')
            }
            ...
            if (guildMap.has(mapKey)) {
                ...
                msg.reply("Bağlantı kesildi.")
            } else {
                msg.reply("Bağlı olmadığı için ayrılamaz.")
            }
        } else if (msg.content.trim().toLowerCase() == _CMD_HELP) {
            msg.reply(getHelpString());
        }
        else if (msg.content.trim().toLowerCase() == _CMD_DEBUG) {
            console.log('hata ayıklama modunu değiştirme')
            let val = guildMap.get(mapKey);
            if (val.debug)
                val.debug = false;
            else
            ...
            if (SPEECH_METHOD === 'witai') {
              ...
              msg.reply('Hata: Bu özellik yalnızca Google içindir')
            }
        }
    } catch (e) {
        console.log('discordClient message: ' + e)
        msg.reply('Hata#180: Bir şeyler ters gitti, tekrar deneyin veya bu durum devam ederse geliştiricilerle iletişime geçin.');
    }
})
...
async function connect(msg, mapKey) {
    try {
        ...
        if (!voice_Channel) return msg.reply("Hata: Ses kanalı mevcut değil!");
        let text_Channel = await discordClient.channels.fetch(msg.channel.id);
        if (!text_Channel) return msg.reply("Hata: Metin kanalı mevcut değil!");
        ...
        msg.reply('Hata: ses kanalınıza katılamıyor.');
        throw e;
    }
}
...
if (SPEECH_METHOD === 'vosk') {
  ...
  recs = {
    'tr': new vosk.Recognizer({model: new vosk.Model('vosk_tr'), sampleRate: 48000}),
...
function process_commands_query(txt, mapKey, user) {
    if (txt && txt.length) {
        let val = guildMap.get(mapKey);
        user.send(user.username + ' adlı kullanıcı diyor ki: ' + txt)
    }
}
...
const gspeechclient = new gspeech.SpeechClient({
  projectId: 'vcxbbhfg',
  keyFilename: 'vcxbbhfg-ce84bcb0e6c8.json'
});
